...
export default function EditLeadPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { register, control, handleSubmit, reset, formState: { errors } } = useForm<LeadFormValues>({
  ...
  useEffect(() => {
    if (lead) {
      // Convert needsMappingAnswers to ans_ format
      const prefilledAnswers: Record<string, any> = {};
      if (lead.needsMappingAnswers) {
        Object.keys(lead.needsMappingAnswers).forEach(key => {
          prefilledAnswers[`ans_${key}`] = lead.needsMappingAnswers[key];
        });
      }
      ...
      reset({
        ...
        needsMappingAnswers: prefilledAnswers,
      });
    }
  }, [lead, reset]);
  ...
  const mutation = useMutation({
    mutationFn: (data: LeadFormValues) => apiFetch(`/crm/leads/${id}`, {
      method: 'PATCH',
  ...
  if (isLoadingLead) {
    return (
      <div className="flex items-center justify-center p-12 text-slate-500">
        <Loader2 className="w-8 h-8 animate-spin mr-2" /> Carregando lead...
      </div>
    );
  }
    ...
    <RequirePermissions permissions={['crm:manage']}>
      <div className="max-w-7xl mx-auto space-y-6 pb-12">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h1 className="text-3xl font-bold tracking-tight text-slate-800">Editar Lead</h1>
            <p className="text-muted-foreground">Atualize os dados do lead.</p>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit(onSubmit, (errs) => {
          ...
          <div className="flex justify-end gap-4 bg-white p-4 rounded-xl border border-border shadow-sm sticky bottom-4">
            ...
            <Button type="submit" size="lg" className="w-48 shadow-lg shadow-primary/30" disabled={mutation.isPending}>
              {mutation.isPending ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" /> Atualizando...</> : 'Atualizar Lead'}
            </Button>
          </div>
          
          {mutation.isError && (
            <p className="text-destructive text-center font-medium bg-red-50 p-3 rounded-md border border-red-200 mt-4">
              Erro ao atualizar: {mutation.error.message}
            </p>
          )}
        </form>
      </div>
